...
class LoginDialog(QDialog):
    ...
    def get_access_token(self, code: str):
        ...
        try:
            ...
            client_id = self.app.config.get("avito_client_id")
            client_secret = self.app.config.get("avito_client_secret")
            redirect_uri = self.app.config.get("avito_redirect_uri")
            
            if not client_id or not client_secret:
                self.status_label.setText("❌ Client ID или Secret не настроены")
                self.status_label.setStyleSheet("color: #FF6B6B;")
                return None
    ...
    def login(self):
        ...
        if not auth_code:
            self.status_label.setText("❌ Введите код авторизации")
            self.status_label.setStyleSheet("color: #FF6B6B;")
            return
        ...
        if user:
            ...
            self.user = user
            self.status_label.setText("✅ Пользователь найден!")
            self.status_label.setStyleSheet("color: #00A651;")
        else:
            user_id = self.app.user_repo.create(username, user_id_avito)
            if user_id:
                self.user = self.app.user_repo.get_by_id(user_id)
                if self.user:
        ...
        if self.user and auth_code:
            ...
            if tokens:
                ...
                if self.app.sync_service:
                    self.app.sync_service.set_access_token(access_token)
                    self.app.sync_service.set_user(user_id_avito)
